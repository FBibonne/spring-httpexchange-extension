...
public class SimpleExampleTest {
    ...
    public static final String BASE_HOST_PORT = "localhost:" + BASE_PORT;
    public static final String BASE_URL = "http://" + BASE_HOST_PORT;
    ...
    private void prepareWireMockWithoutOption(String id) throws IOException {
        prepareWireMockWithOption(id, UnaryOperator.identity());
    }
    ...
    private void prepareWireMockWithCache(String id) throws IOException {
        prepareWireMockWithOption(id,
                response -> response.withHeader("Cache-Control", CacheControl.maxAge(1L, TimeUnit.HOURS).getHeaderValue(),
                        CacheControl.empty().mustRevalidate().getHeaderValue(),
                        CacheControl.empty().cachePrivate().getHeaderValue())
        );
    }

    private void prepareWireMockWithOption(String id, UnaryOperator<ResponseDefinitionBuilder> option) throws IOException {
        ResponseDefinitionBuilder responseDefinitionBuilder = ok()
                .withHeader("Content-Type", ContentType.APPLICATION_JSON.getMimeType())
                .withBody(regionsResponse());
        stubFor(get("/nomenclature/" + id).willReturn(option.apply(responseDefinitionBuilder)));
    }

    @Test
    void withDefaults_checkHttpCompressionIsEnabled() throws IOException {
        var id = "regions2019";
        prepareWireMockWithoutOption(id);
        (new AnnotationConfigApplicationContext(SimpleConfig.class)).getBean(SimpleRegionHttpInterface.class).getRegions(id);
        WireMock.verify(newRequestPattern().withHeader("Accept-Encoding", or(containing("gzip"), containing("defalte"), containing("br"))));
    }
    ...
    static String regionsResponse() throws IOException {
